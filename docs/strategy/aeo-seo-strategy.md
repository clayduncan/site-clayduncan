...
| Local                 | "Mortgage broker near me Huntsville"            | ProfessionalService `areaServed` + GBP                               |
...
| 18  | ProfessionalService schema with service areas on personal sites       | High       | Hours   | Weeks           | Low  | Service areas verified     | Stable       | Free     |
...
- Contact (Person + ProfessionalService schema, phone, calendar)
...
**Schema baseline:** Person (canonical), Organization, ProfessionalService, Service, WebSite, WebPage, BreadcrumbList, FAQPage on FAQ pages, Article on pillar pages, Course or LearningResource on specialty pillar
...
**Schema baseline:** Organization, ProfessionalService (without NMLS identifier), Service, WebSite, WebPage, BreadcrumbList. Person references by `@id` to personal sites only.
